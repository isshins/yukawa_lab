...
# モデル作成
def model_sequential(activation, layers):
    ...
    model_add_block(model, layers, activation)
...
for layer in range(12, 23):
    # モデルのコンパイル
    model = model_sequential(act, layer)
    model.summary()
    ...
    # 学習済みモデルを保存
    model.save(f'{dataset}_{act}_{layer}.h5')
    train_acc = history.history['accuracy'][9]
    train_loss = history.history['loss'][9]
    val_acc = history.history['val_accuracy'][9]
    val_loss = history.history['val_loss'][9]
    layers_list.append([train_loss, train_acc, val_loss, val_acc])

layers_list = np.array(layers_list).T
np.savetxt(f'./{dataset}_{act}_layers.csv', layers_list)
