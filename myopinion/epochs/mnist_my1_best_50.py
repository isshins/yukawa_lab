...
class My1_best(Activation):
    def __init__(self, activation, **kwargs):
        super().__init__(activation, **kwargs)
        self.__name__ = 'My1_best'


def my1_best(inputs):
    return tf.keras.activations.relu(inputs) - 0.5

get_custom_objects().update({'My1_best': My1_best(my1_best)})
...
# モデルのコンパイル
model = model_sequential("My1_best")
model.summary()
...
np.savetxt('./mnist_my1_best_50.csv', [loss, acc, val_loss, val_acc])
