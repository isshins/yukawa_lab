...
for i in range(21):
    print(f"now act is myopinion1_{i}")
    def myopinion(inputs, alpha = i * 0.1):
        return tf.keras.activations.relu(inputs) - alpha
    
    get_custom_objects().update({'Myopinion1': Myopinion(myopinion)})

    # モデルのコンパイル
    model = model_sequential(f"Myopinion1")
    model.summary()
    ...
    np.savetxt(f'./mnist_myopinion1_{i}.csv', [loss, acc, val_loss, val_acc])
