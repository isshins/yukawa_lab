...
class Fts09(Activation):

    def __init__(self, activation, **kwargs):
        super(Fts09, self).__init__(activation, **kwargs)
        self.__name__ = 'Fts09'


def fts09(inputs):
    return inputs * tf.math.sigmoid(tf.keras.activations.relu(inputs*0.9))

get_custom_objects().update({'Fts09': Fts09(fts09)})
