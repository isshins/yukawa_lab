...
class Fts_t(Activation):

    def __init__(self, activation, **kwargs):
        super(Fts_t, self).__init__(activation, **kwargs)
        self.__name__ = 'Fts_t'


def fts_t(inputs):
    return inputs * tf.math.sigmoid(tf.keras.activations.relu(inputs)) - 0.1

get_custom_objects().update({'Fts_t': Fts_t(fts_t)})
